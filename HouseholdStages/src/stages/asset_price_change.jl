...
Returns a bundled [`WealthChangeStage`](@ref) whose `wealth_post`
closure is
...
behaviour (`backward!`, `forward!`, `with_eltype`, `lift_gpu`, the
forward-mode AD path) applies unchanged. This constructor is sugar for
the common case, not a parallel implementation; it does not introduce
its own Spec/Buffer/Stage trichotomy.

The "previous price" enters through `env.q_last`. There is no
per-stage runtime cache of `q_last`: the closure reads it from `env`
on every backward pass, and it is the outer driver's responsibility to
seed `env.q_last` from the previous period's `env.q` (or however the
transition path defines "previous").
...
backward!(stage, V_post, env)
```
"""#CLAUDE This docstring is bloated
