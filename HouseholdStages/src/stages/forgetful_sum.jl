"""
    ForgetfulSum{T, Nin, Nout, LIn, LOut} <: AbstractStage
...
layout difference (no V/θ dependence; cache = `nothing`). Backward
...
(flow payoff `r = 0`).
"""
struct ForgetfulSum{T<:Real, Nin, Nout,
...
"""
    ForgetfulSum(layout; forget_axis, element_type=Float64,
                          V_start=nothing, Λ_end=nothing) -> ForgetfulSum
...
[`ProductStage`](@ref).
"""
function ForgetfulSum(layout::StateLayout;
                      forget_axis::Symbol,
    ...
    return ForgetfulSum{T, length(dims_in), length(dims_out),
...
static_env_deps(::Type{<:ForgetfulSum}) = NamedTuple()

# K = sum-along-axis; no materialized kernel data, no scratch.
function allocate(stage::ForgetfulSum{T,Nin,Nout},
                  ::Type{T2} = T) where {T,Nin,Nout,T2}
...
function backward!(stage::ForgetfulSum{T,Nin,Nout},
                   V_end::AbstractArray{T,Nout},
                   env, kernel, scratch) where {T,Nin,Nout}
    dims_out = layout_size(stage.output_layout)
    @assert size(V_end) == dims_out
    shape          = _insert_singleton(dims_out, stage.forget_dim)
    V_end_reshaped = reshape(V_end, shape)
    stage.V_start .= V_end_reshaped
...
function forward!(stage::ForgetfulSum{T,Nin,Nout},
                  Λ_start::AbstractArray{T,Nin},
                  kernel, scratch,
                  moments = nothing) where {T,Nin,Nout}
    dims_in  = layout_size(stage.input_layout)
    dims_out = layout_size(stage.output_layout)
    @assert size(Λ_start) == dims_in
    shape           = _insert_singleton(dims_out, stage.forget_dim)
    Λ_end_reshaped  = reshape(stage.Λ_end, shape)
