...
@testset "product — backward & forward run per-component" begin
    ...
    s = MarkovAlong(layout; axis = :z, transition = P)
    ps = product(s, s; axis = :group)
    caches, scratches = allocate(ps)

    V_end = ones(2, 2)
    V_start = backward!(ps, V_end, NamedTuple(), caches, scratches)
    ...
    Λ_end = forward!(ps, Λ_start, caches, scratches)
...
@testset "product — view-based fused tensor for uniform MarkovAlong" begin
    # With use_views=true (default), the rebuilt MarkovAlong components
...
@testset "product — ForgetfulSum view-mode (V_start views; Λ_end allocates fresh)" begin
    layout = StateLayout(
        StateAxis(:w, continuous_grid([0.0, 1.0, 2.0])),
        StateAxis(:t, categorical([:a, :b])),
    )
    s1 = ForgetfulSum(layout; forget_axis = :t)
    s2 = ForgetfulSum(layout; forget_axis = :t)
    ps = product(s1, s2; axis = :group)
    ...
    # product axis); ForgetfulSum allocates Λ_end fresh in this case.
    @test ps.components[1].Λ_end isa Array
end

@testset "product — choice stages now use view-mode too" begin
    # Argmax / LogitChoice / GridSavings were ported to the view
    # protocol in round 4. Verify Argmax components have SubArray
    ...
    nsx = (cell, a) -> a
    s1 = Argmax(layout; choice_axis = :a, flow_payoff = fp, next_state_idx = nsx)
    s2 = Argmax(layout; choice_axis = :a, flow_payoff = fp, next_state_idx = nsx)
    ps = product(s1, s2; axis = :group)
    @test ps.components[1].V_start isa SubArray
