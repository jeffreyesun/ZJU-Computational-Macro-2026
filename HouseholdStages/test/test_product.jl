...
@testset "product — uniform 2-component, axis = :group" begin
    ...
    @test ps.spec.axis_size == 2
    @test layout_size(ps.spec.input_layout) == (2, 2)
    @test layout_size(ps.spec.output_layout) == (2, 2)
end
...
@testset "replicate_age — N copies along :age" begin
    ...
    @test ages.spec.axis_size == 5
    @test layout_size(ages.spec.input_layout) == (2, 5)
end
